...
def mouse_left_drag(x1, x2, y1, y2):
    ...
    length = math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2)
    # print(length)
    if length < 40 and drag == False:
        drag = True
        pag.mouseDown()

    if length > 40 and drag == True:
        drag = False
        pag.mouseUp()
        ...
        if results.multi_hand_landmarks:
            ...
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                for i in hand_landmarks.landmark:
                    ...
                    x12 = hand_landmarks.landmark[12].x * 1920  # 取得食指第三端點 X座標
                    y12 = hand_landmarks.landmark[12].y * 1080  # 取得食指第三端點 Y座標
                ...
                x12_output = round(x12, 0)
                y12_output = round(y12, 0)
                ...
                if finger_points and handedness.classification[0].label[0:] == 'Right':
                    ...
                    thread3.start()
                    thread4 = threading.Thread(target=right_click, args=(x8_output, x12_output, y8_output, y12_output))
                    thread4.start()
